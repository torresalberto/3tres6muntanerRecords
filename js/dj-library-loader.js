...
const DJLibrary = {
  ...
  async loadAll() {
    try {
      ...
      try {
        ...
        this.crossRefs = { shared_artists: [], shared_tracks: [], shared_labels: [] };
      }
    } catch (e) {
        ...
        <section class="dj-profile-section">
          <div class="dj-profile-card">
            ...
            ${sets
              ...
              ${
                set.curious_facts && Object.keys(set.curious_facts).length
                  ? `<div class="facts-section"><div class="section-label">📊 Datos</div><div class="facts-grid">${Object.entries(
                      set.curious_facts
                    )
                      .map(
                        ([k, v]) =>
                          `<div class="fact-card"><div class="fact-number">${v}</div><div class="fact-label">${k.replace(/_/g, ' ')}</div></div>`
                      )
                      .join('')}</div></div>`
                  : ''
              }
    ...
    if (el) {
      el.innerHTML = total
        ...
        ${arts
          ...
          .map(
            (a) =>
              `<div style="display:flex;gap:0.5rem;padding:0.3rem 0;font-size:0.8rem;color:rgba(255,255,255,0.6)"><span style="color:#ff9100">●</span><span>${a.artist}</span><span style="color:rgba(255,255,255,0.3)">→</span><span style="color:#ff4d00">${a.djs.join(' & ')}</span></div>`
          )
          .join('')}
        ${tracks
          .slice(0, 3)
  ...
  renderStats() {
    ...
    const trackCount = Object.values(this.sets).reduce(
      (sum, s) => sum + (s.tracklist || []).length,
      0
    );
    const crossCount =
      (this.crossRefs?.shared_artists?.length || 0) + (this.crossRefs?.shared_tracks?.length || 0);
    statEl.innerHTML = `
