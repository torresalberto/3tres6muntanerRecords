(function () {
  ...
  var GENRE_COLORS = {
    ...
    'Deep House': '#ffb74d',
    'Tech House': '#ff7043',
    'Minimal Techno': '#e0e0e0',
    'Detroit Techno': '#ffab91',
    'Melodic House': '#ffd54f',
    'Melodic Techno': '#ffe082',
    'UK Bass': '#b0bec5',
    Disco: '#fff176',
    'Deep Dub Techno': '#90a4ae',
    Ambient: '#78909c',
    Minimal: '#cfd8dc',
    'Acid House': '#ff5252',
    Funk: '#ffcc80',
  ...
  async function loadData() {
    ...
    djsList.forEach(function (dj) {
      var node = {
        ...
        scale: 1,
        color:
          GENRE_COLORS[dj.genres && dj.genres.length ? dj.genres[0] : 'Unknown'] ||
          GENRE_COLORS.Unknown,
      };
      nodes.push(node);
      nodeMap[dj.id] = node;
    });
  ...
  function createSVGElements() {
    ...
    links.forEach(function (l) {
      ...
      line.setAttribute(
        'stroke',
        l.type === 'track' ? 'rgba(255,255,255,0.35)' : 'rgba(255,255,255,0.12)'
      );
  ...
  function render() {
    ...
    for (var li = 0; li < 3; li++) {
      ...
      var latHeight =
        Math.sqrt(RADIUS * RADIUS - ly * ly) * Math.abs(COS(rotX)) * latProj.scale * 0.3;
    ...
    for (var pi = 0; pi < projected.length; pi++) {
      ...
      if (hoveredNode) {
        for (var ci = 0; ci < links.length; ci++) {
          if (
            (links[ci].source === hoveredNode.id && links[ci].target === nd.id) ||
      ...
      if (selectedNode) {
        for (var ci2 = 0; ci2 < links.length; ci2++) {
          if (
            (links[ci2].source === selectedNode.id && links[ci2].target === nd.id) ||
  ...
  function showTooltip(e, node) {
    if (!tooltipEl) {
      ...
      tooltipEl.style.cssText =
        'position:fixed;pointer-events:none;z-index:1000;background:rgba(0,0,0,0.9);border:1px solid rgba(255,77,0,0.3);border-radius:8px;padding:8px 12px;color:white;font-size:12px;font-family:Inter,sans-serif;max-width:200px;box-shadow:0 4px 20px rgba(0,0,0,0.5);';
  ...
  function showSidePanel(node) {
    ...
    html +=
      '<div style="color:rgba(255,255,255,0.6);margin-bottom:16px;font-size:0.85rem;">' +
      node.genre +
      '</div>';
    ...
    if (sharedTracks.length > 0) {
      html +=
        '<div style="font-size:11px;color:rgba(255,255,255,0.5);margin-bottom:8px;letter-spacing:1px;text-transform:uppercase;">Shared Tracks (' +
        sharedTracks.length +
        ')</div>';
      html += '<div style="max-height:350px;overflow-y:auto;margin-bottom:16px;">';
      sharedTracks.forEach(function (st) {
        var trackName = st.track;
        var otherDjs = st.djs.filter(function (d) {
          return d !== node.id;
        });
        var setsForTrack = st.sets || [];

        html +=
          '<div style="margin-bottom:12px;padding:8px;background:rgba(255,77,0,0.06);border-radius:8px;border-left:3px solid #ff4d00;">';
        html +=
          '<div style="color:#ff4d00;font-size:13px;font-weight:600;margin-bottom:4px;">' +
          trackName +
          '</div>';
        html +=
          '<div style="font-size:11px;color:rgba(255,255,255,0.7);margin-bottom:4px;">Also played by:</div>';
        otherDjs.forEach(function (djId) {
          var djNode = nodeMap[djId];
          var djName = djNode ? djNode.name : djId;
          html +=
            '<div style="color:rgba(255,255,255,0.9);font-size:12px;padding:1px 0;">• ' +
            djName +
            '</div>';
        });
        ...
        if (setsForTrack.length > 0) {
          html +=
            '<div style="font-size:10px;color:rgba(255,255,255,0.4);margin-top:4px;">Sets:</div>';
          setsForTrack.forEach(function (s) {
            ...
            html +=
              '<div style="color:rgba(255,255,255,0.5);font-size:10px;padding:1px 0;">— ' +
              setTitle;
            if (setArtistName)
              html += ' <span style="color:rgba(255,255,255,0.3);">(' + setArtistName + ')</span>';
            html += '</div>';
          });
        }
        html += '</div>';
    ...
    if (sharedArtists.length > 0) {
      html +=
        '<div style="font-size:11px;color:rgba(255,255,255,0.5);margin-bottom:8px;letter-spacing:1px;text-transform:uppercase;">Shared Artists (' +
        sharedArtists.length +
        ')</div>';
      html += '<div style="max-height:250px;overflow-y:auto;margin-bottom:12px;">';
      sharedArtists.forEach(function (sa) {
        var otherDjs = sa.djs.filter(function (d) {
          return d !== node.id;
        });
        var tracks = sa.tracks || [];
        html +=
          '<div style="margin-bottom:8px;padding:8px;background:rgba(255,145,0,0.06);border-radius:8px;border-left:3px solid #ff9100;">';
        html +=
          '<div style="color:#ff9100;font-size:13px;font-weight:600;margin-bottom:2px;">' +
          sa.artist +
          '</div>';
        if (tracks.length > 0) {
          html +=
            '<div style="font-size:11px;color:rgba(255,255,255,0.6);">Tracks: ' +
            tracks.join(', ') +
            '</div>';
        }
        html +=
          '<div style="font-size:11px;color:rgba(255,255,255,0.5);margin-top:3px;">Also played by: ';
        html += otherDjs
          .map(function (djId) {
            var djNode = nodeMap[djId];
    ...
    if (sharedTracks.length === 0 && sharedArtists.length === 0) {
      html +=
        '<div style="color:rgba(255,255,255,0.3);font-size:13px;text-align:center;padding:20px 0;">No shared tracks or artists found in the library yet.</div>';
    }

    html +=
      '<a href="dj/' +
      node.id +
      '.html" style="display:inline-block;margin-top:12px;padding:8px 16px;background:rgba(255,77,0,0.1);border:1px solid rgba(255,77,0,0.3);border-radius:6px;color:#ff4d00;text-decoration:none;font-size:12px;font-weight:600;text-align:center;">Ver en DJ Library →</a>';
