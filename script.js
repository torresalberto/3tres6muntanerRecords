...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const Checkout = {
    ...
    init() {
      ...
      document.getElementById('checkoutShippingForm')?.addEventListener('submit', (e) => {
        ...
        this.shippingCost =
          shippingMethod === 'express' ? 199 : shippingMethod === 'pickup' ? 0 : 99;
      ...
      // Payment buttons
      document
        .getElementById('payWithMP')
        ?.addEventListener('click', () => this.payWithMercadoPago());
      document
        .getElementById('payWithStripe')
        ?.addEventListener('click', () => this.payWithStripe());
      document
        .getElementById('payWithPayPal')
        ?.addEventListener('click', () => this.payWithPayPal());
      document
        .getElementById('payWithTransfer')
        ?.addEventListener('click', () => this.payWithTransfer());
    },
  ...
  const AudioPlayer = {
    ...
    startMusic(videoId = null, title = null, startMuted = false) {
      ...
      trackEvent('audio_play', {
        source: videoId ? 'vinyl_track' : '3tres6_radio',
  ...
  const HeroPlaylist = {
    ...
    populateFromInventory(listings) {
      ...
      // Render tracks
      tracksContainer.innerHTML = playlistItems
        .map(
          (track) => `
  ...
  const CalendarLiveTabs = {
    ...
    switchTab(platform) {
      ...
      if (platform === 'youtube') {
        ...
        iframe.src =
          'https://www.youtube.com/embed/live_stream?channel=UCxxxxxxxxxxxxxx&autoplay=0&rel=0&modestbranding=1';
        iframe.allow =
        ...
        if (placeholder) {
          placeholder
            .querySelector('.follow-live-btn.yt')
  ...
  const ExitIntent = {
    ...
    init() {
      ...
      document.addEventListener('mouseout', (e) => {
        if (e.clientY < CONFIG.exitIntent.sensitivity && e.relatedTarget === null) {
          this.show();
        }
      });
