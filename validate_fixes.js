...
testUrls.forEach(({ url, expected }) => {
  ...
  test(
    `QuickView extractYouTubeId("${url.substring(0, 50)}...")`,
    result === expected,
...
testUrls.forEach(({ url, expected }) => {
  ...
  test(
    `HeroPlaylist extractYouTubeId("${url.substring(0, 50)}...")`,
    result === expected,
    `got: ${result}, expected: ${expected}`
  );
});
...
const carouselWrapperMatch = cssContent.match(
  /\.instagram-carousel-wrapper\s*\{[^}]*padding:\s*([^;]+);/
);
if (carouselWrapperMatch) {
  ...
  test(
    '.instagram-carousel-wrapper padding found',
    false,
    'Could not find .instagram-carousel-wrapper padding'
  );
}
...
const heroPlaylistExtractMatch = jsContent.match(
  /HeroPlaylist[\s\S]*?extractYouTubeId\(url\)\s*\{([^}]*)\}/
);
if (heroPlaylistExtractMatch) {
  ...
  test(
    'HeroPlaylist.extractYouTubeId uses match[7] (not match[8])',
    hasMatch7 && !hasMatch8,
    heroPlaylistExtractMatch[1].trim()
  );
} else {
  test(
    'HeroPlaylist.extractYouTubeId found',
    false,
    'Could not find HeroPlaylist.extractYouTubeId'
  );
}
...
test(
  'blog-card-image is an <a> tag',
  !!blogCardImageHtmlMatch,
  blogCardImageHtmlMatch ? blogCardImageHtmlMatch[0] : 'not found'
);

// Check live-streams-banner is inside calendar-section
const calendarSectionMatch = htmlContent.match(
  /class="section calendar-section"[\s\S]*?class="live-streams-banner"/
);
...
const liveEmbedMatch = htmlContent.match(
  /class="section calendar-section"[\s\S]*?class="live-embed-container"/
);
...
const instagramMatch = htmlContent.match(
  /class="section instagram-section"[\s\S]*?class="instagram-carousel-wrapper"/
);
