const CamelotWheel = {
  ...
  selectKey: function (keyId) {
    ...
    document.querySelectorAll('.wheel-segment').forEach((seg) => {
      seg.classList.remove(
        'selected',
        'compatible-perfect',
        'compatible-energy-up',
        'compatible-energy-down',
        'compatible-energy-shift',
        'compatible-adjacent'
      );
      seg.style.filter = '';
      seg.style.opacity = '1';
    ...
    compatible.forEach((comp) => {
      ...
      if (compSegment) {
        compSegment.classList.add(
          `compatible-${comp.type.replace('Up', '-up').replace('Down', '-down').replace('Shift', '-shift')}`
        );
  ...
  updateCenterLabel: function (key) {
    ...
    if (centerLabel && key) {
      ...
      centerLabel.setAttribute(
        'fill',
        key.letter === 'A' ? this.colors.a[key.number] : this.colors.b[key.number]
      );
  ...
  clearSelection: function () {
    ...
    if (keyInfo) {
      keyInfo.innerHTML =
        '<p class="placeholder-text">Click on a key in the wheel to see compatible tracks</p>';
    }
